...
app.post('/create-letter', async (req, res) => {
  try {
    ...
    db[id] = {
      msg: clean,
      hash: passphrase ? hash(passphrase) : null,
      created: Date.now()
    };
    saveLetters(db);
...
app.get('/letter/:id', (req, res) => {
  res.redirect(`/view.html/${req.params.id}`);
});
...
async function sendEmail(to, link, msg) {
  if (!process.env.SMTP_HOST) return; // no-op if SMTP not set
